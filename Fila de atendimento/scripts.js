...
function automatizarFila(filaDePacientes, tipoDeOperacao, nome) {
    if (tipoDeOperacao === "agendar") {
        filaDePacientes.push(nome)

    } else {
        filaDePacientes.shift()

    } return console.log(filaDePacientes);

}
automatizarFila(pacientes, "agendar", "Jessé");
automatizarFila(pacientes, "atender");